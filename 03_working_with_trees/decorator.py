...
class TreeDecorator:
    ...
    def _find_taxonomy(self, taxonomy_list):
        '''
    ...
    def decorate(self, output):
        ...
        gg_prefixes = set(["k__", 'p__', 'c__', 'o__', 'f__', 'g__', 's__'])
        ...
        for node in self._nodes():
            ...
            children_taxonomy = [self.taxonomy[t.name] 
                                    for t in children]
            node_tax = self._find_taxonomy(children_taxonomy)
            
            tax_list = []
            for index, rank in enumerate(node_tax):
                if len(rank) == 1:
                    tax=list(rank)[0]
                    if tax not in gg_prefixes:
            ...
            if len(tax_list) >= 1:
                ...
                if parent.is_root():
                    ...
                    current_index = self._get_tax_index(node.ancestors())
                    if len(tax_list) <= current_index:
                        ...
                        if len(tax_list) == placement_depth:
                            tax_string = '; '.join(tax_list[current_index:])
                        elif len(node.children) == len(list(node.tips())):
                            tax_string = '; '.join(tax_list[current_index:])
                        else:
                            tax_string = tax_list[current_index]

                if tax_string in self.encountered_taxonomies:
                    idx=1
                    while tax_string in self.encountered_taxonomies:
                        if idx > 1: 
                            tax_string = '%s_%i' % ('_'.join(tax_string.split('_')[:-1]), idx)
                        else:
                            tax_string = tax_string + '_%i' % idx
                        idx += 1
                
                node.name = tax_string
                logging.debug("Renamed node %s" % tax_string)
                self.encountered_nodes[node]=index
                self.encountered_taxonomies.add(tax_string)
...
if __name__ == '__main__':
    ...
    output_options.add_argument('--output', help='Output tree file', type=str, required=True)
