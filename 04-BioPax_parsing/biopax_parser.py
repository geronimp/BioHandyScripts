...
ko_to_ec_file = sys.argv[1]
input_biopax = sys.argv[2]
sep = '\t'
...
def parse_ko_to_ec(input_file):
	ec_to_ko = dict()

	for line in open(input_file):
		ko, ec = line.strip().split('\t')
...
ec_to_ko = parse_ko_to_ec(ko_to_ec_file)
...
print("%s\t%s" % (pathway_name, ' '.join(pathway_definition)))